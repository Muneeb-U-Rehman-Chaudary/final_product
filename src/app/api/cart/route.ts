...
export async function GET(request: NextRequest) {
  try {
    await connectDB();
      ...
      const userCart = await Cart.findOne({ userId: session.user.id }).lean() as any;
      ...
      const cartProducts = await Product.find({ productId: { $in: productIds as any } }).lean();

      const cartWithDetails = items.map((item: any) => {
        const product = cartProducts.find(p => Number(p.productId) === Number(item.productId)) as any;
...
export async function POST(request: NextRequest) {
    try {
      ...
      const product = await Product.findOne({ productId: Number(productId) as any }).lean();
...
export async function DELETE(request: NextRequest) {
    try {
      ...
      userCart.items = (filteredItems as any);
      userCart.updatedAt = new Date();
      await userCart.save();
